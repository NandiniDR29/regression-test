...
public class UploadFormTabPage {
    Actions builder;
    Robot robot;
    WebDriver driver;
    ...
    @FindBy(xpath = "(//a[contains(@href,'/forms/upload?')])[1]")
    ...
    public UploadFormTabPage(WebDriver driver, WebDriverWait wait, Robot robot, Actions builder,
                             Properties prop) {
...
//            if(tab.getText().equalsIgnoreCase("Download form")||tab.getText().equalsIgnoreCase("Upload form")){
            if(tab.getText().equalsIgnoreCase("Document library")||tab.getText().equalsIgnoreCase("Upload form")){
                ...
                toFailScenarios("Document library and Upload form tabs are not available in Form Tab "); }
        }
        if (i!=0){
...
//        ClickSubmitButton.click();
    ...
    public void fileUpload() throws Exception {
        ...
        uploadFile.click();
        File file = new File(PropertyProvider.getProperty(TestProperties.UPLOAD_FILES_LOCATION), "upload123457.pdf");
