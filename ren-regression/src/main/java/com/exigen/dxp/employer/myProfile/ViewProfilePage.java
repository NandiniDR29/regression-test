...
public class ViewProfilePage {
    ...
    @FindBy(xpath = "(//a[@class='item_3LVDELaP'])[1]")
    ...
    public ViewProfilePage(WebDriver driver, WebDriverWait wait, Robot robot, Actions builder,
                           Properties prop) {
...
//Validating of radio buttons
        ...
        if(radiobutton.contains(false))
            toFailScenarios("Contact preferences radio buttons are not displayed");
        else
            reportLog("Contact preferences radio buttons are displayed", 1);
    }
