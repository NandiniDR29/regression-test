...
package com.exigen.modules.policy.common.pages;
...
public class PolicySummaryPage extends SummaryPage {
    ...
    public static StaticElement labelParticipantName=new StaticElement(By.xpath("//span[@id='productContextInfoForm:policyDetail_customerDisplayNameText']"));
    ...
    public static StaticElement labelError = new StaticElement(By.id("productConsolidatedViewForm:logTable:0:errorType"));
    ...
    public static TableExtended<RoleSummary> tableEnhancedAccidentRoleSummary = new TableExtended<>("Enhanced pName - Role Summary",
            By.xpath("//td[text()='Enhanced pName - Role Summary']//ancestor::table[1]//following-sibling::div[1]//table"));
    ...
    public static void expandEnrollmentProcessingResultsTable() {
        ...
        if (new StaticElement(By.xpath("//input[@id='productConsolidatedViewForm:enrollmentProcessingTogglePanel-value']")).getAttribute("value").equals("false")) {
            new Link(By.xpath("//div[@id='productConsolidatedViewForm:enrollmentProcessingTogglePanel']")).click(Waiters.AJAX.then(Waiters.SLEEP(20000)));
            int endtime = CommonGenericMethods.waitingHours(360);
            int currentTime= CommonGenericMethods.currentTimeIHours(0);
            int addedTime=0;
            int i=0;
            String[] inprocessCount = CommonBaseTest.driver.findElement(By.xpath("(//table[@id='productConsolidatedViewForm:enrollmentStatisticsGrid']//span[contains(@id,'productConsolidatedViewForm:')])[2]")).getText().split(regex);

//            while((CommonBaseTest.driver.findElements(By.xpath("//span[contains(text(),'Completed')]")).size()==0&&!(CommonBaseTest.driver.findElement(By.xpath("(//*[contains(@id,'productConsolidatedViewForm:j_id_')]/text())[2]")).getText().equalsIgnoreCase("0")))) {
            while((CommonBaseTest.driver.findElements(By.xpath("//span[contains(text(),'Completed')]")).size()==0||Integer.parseInt(inprocessCount[1])!=0)) {
//                CommonBaseTest.driver.findElement(By.xpath("//input[@id='productConsolidatedViewForm:enrollmentRecordResultTable:fileId:filter']")).sendKeys(CommonBaseTest.prop.getProperty("ReferanceFileName"));
//              Waiters.SLEEP(9000).go();
                inprocessCount = CommonBaseTest.driver.findElement(By.xpath("(//table[@id='productConsolidatedViewForm:enrollmentStatisticsGrid']//span[contains(@id,'productConsolidatedViewForm:')])[2]")).getText().split(regex);
                List<WebElement> status = CommonBaseTest.driver.findElements(By.xpath("//tbody[@id='productConsolidatedViewForm:enrollmentRecordResultTable_data']//tr[@role='row']/td[8]//span"));
               if(status.get(i).getText().equalsIgnoreCase("Failed")){
                   int failedSize=0;
                   List<WebElement> pages = CommonBaseTest.driver.findElements(By.xpath("//span[contains(@class,'ui-paginator-page ui-state-default ui-corner-all')]"));
                   for (int j = 1; j < pages.size()-1; j++) {
                         pages.get(j+1).click();
                       Waiters.SLEEP(9000).go();
                       failedSize = CommonBaseTest.driver.findElements(By.xpath("//span[contains(text(),'Failed')]")).size()+failedSize;
                   }
                  CommonGenericMethods.informationPrint(failedSize+" Total lives failed");
               }
               else if(status.get(i).getText().equalsIgnoreCase("Completed")&&Integer.parseInt(inprocessCount[1])==0){
                   break;
               }
               else {
                BrowserController.get().driver().navigate().refresh();
                Waiters.SLEEP(90000).go();
                new Link(By.xpath("//div[@id='productConsolidatedViewForm:enrollmentProcessingTogglePanel']")).click(Waiters.AJAX.then(Waiters.SLEEP(20000)));
                if(currentTime>=60){addedTime=addedTime+currentTime;
                if(i<10){
                    i++;
                }
                else{
                ...
                if(endtime==addedTime){
                    CustomAssertions.assertThat("Cp are still Passed Status");
                }
               }
            }
    ...
    public static void expandEnrollmentProcessingResultsTableToSearchFile() {
        String regex="[^\\d]+";
        if (new StaticElement(By.xpath("//input[@id='productConsolidatedViewForm:enrollmentProcessingTogglePanel-value']")).getAttribute("value").equals("false")) {
            new Link(By.xpath("//div[@id='productConsolidatedViewForm:enrollmentProcessingTogglePanel']")).click(Waiters.AJAX.then(Waiters.SLEEP(20000)));
            CommonBaseTest.driver.findElement(By.xpath("//input[@id='productConsolidatedViewForm:enrollmentRecordResultTable:fileId:filter']")).sendKeys(CommonBaseTest.prop.getProperty("ReferanceFileName"));
        ...
        try {
            RetryService.run(row -> row.isPresent() && row.getCell(8).getValue().equals(enrollmentStatus),
                    () -> {
                        ...
                        return tableEnrollmentProcessingResults.findRow(EnrollmentProcessingResults.NAME.getName(), name);
                    }, StopStrategies.stopAfterAttempt(30), WaitStrategies.fixedWait(10, TimeUnit.SECONDS));
        } catch (RuntimeException e) {
            if (tableEnrollmentProcessingResults.getRow(EnrollmentProcessingResults.NAME.getName(), name).isPresent()) {
                throw new IstfException(String.format("'Enrollment Status': Expected: 'Completed', but Actual '%s'",
                        tableEnrollmentProcessingResults.findRow(EnrollmentProcessingResults.NAME.getName(), name).getCell(8).getValue()), e);
            } else {
