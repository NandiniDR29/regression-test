...
import com.exigen.modules.policy.gb_dn.masterpolicy.metadata.PlanDefinitionIssueActionTabMetaData;
            ...
            private void fillSection(TestData td) {
                td.getTestDataList(name).forEach(plan -> {
                    if (plan.containsKey("PlanKey")) {
                        tablePlanSelection.getRow(PolicyConstants.PolicyPlanTable.PLAN, plan.getValue("PlanKey")).getCell(6).controls.links.get("Change").click();
                    }
