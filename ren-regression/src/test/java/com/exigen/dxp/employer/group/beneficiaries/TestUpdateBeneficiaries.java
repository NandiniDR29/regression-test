...
public class TestUpdateBeneficiaries extends DXPBaseTest {
    @Test(priority = 17)
    public void testUpdateBeneficiaries() throws IOException {
        logger = extent.startTest("TestUpdateBeneficiaries");
        navigateToMainPage();
    ...
    public void commonSteps(String subscriberID,  BeneficiariesPage bp) throws IOException {
        ...
        for (int i = 0; i <bp.rowsSize-1 ; i++) {
            bp.clickOnUpDateBeneficiary();
            reportLog("Verify Update Beneficiaries form page Title",1);
            bp.verifyUpdateBeneficiariesTitle();}
    }
}