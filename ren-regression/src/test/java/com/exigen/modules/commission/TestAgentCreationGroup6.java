...
public class TestAgentCreationGroup6 extends CommissionStrategyBaseTest {
    @Test(priority = 0)
    public void testAgentCreationGroup6() throws IOException {
    logger = extent.startTest(groupNo + " Agent Creation for Group6");
    prop.load(fileInput);
    reportLog("Navigate Admin Section",0);
    driver.findElement(By.id("logoutForm:switchToAdmin")).click();
    reportLog("Creating Agent",1);
    String agencyName = agency.createAgency(TestDataProvider.getDefaultTestDataProvider().get("modules/platform/admin/agencyvendor/agency", "DataGatherGroup6").getTestData(TestDataKey.DEFAULT_TEST_DATA_KEY));
    ...
    TestData flatTestData = TestDataProvider.getDefaultTestDataProvider().get("/modules/commission/commissionstrategy", "CommissionStrategyGroup6").getTestData("Flat_CommissionRule")
    ...
    createCommissionStrategy(TestDataProvider.getDefaultTestDataProvider().get("/modules/commission/commissionstrategy", "CommissionStrategyGroup6").getTestData("TestDataSTD")
            .adjust(gbCommissionRuleTab.getMetaKey(), ImmutableList.of(flatTestData)), false);

    reportLog("Add Strategy for Term Life",1);
    createCommissionStrategy(TestDataProvider.getDefaultTestDataProvider().get("/modules/commission/commissionstrategy", "CommissionStrategyGroup6").getTestData("TestDataLife")
            .adjust(gbCommissionRuleTab.getMetaKey(), ImmutableList.of(flatTestData)), false);

    reportLog("Add Strategy for Vision",1);
    createCommissionStrategy(TestDataProvider.getDefaultTestDataProvider().get("/modules/commission/commissionstrategy", "CommissionStrategyGroup6").getTestData("TestDataVision")
            .adjust(gbCommissionRuleTab.getMetaKey(), ImmutableList.of(flatTestData)), false);

    reportLog("Agent Name for Group6="+ agencyName, 1);}
        catch (Exception e){
