...
public class TestAgentCreationGroup5 extends CommissionStrategyBaseTest {
    @Test(priority = 0)
    public void testAgentCreationGroup5() throws IOException {
        logger = extent.startTest(groupNo + " Agent Creation for Group5");
        prop.load(fileInput);
        reportLog("Navigate Admin Section",0);
        driver.findElement(By.id("logoutForm:switchToAdmin")).click();
        reportLog("Creating Agent",1);
        String agencyName = agency.createAgency(TestDataProvider.getDefaultTestDataProvider().get("modules/platform/admin/agencyvendor/agency", "DataGatherGroup5").getTestData(TestDataKey.DEFAULT_TEST_DATA_KEY));
        ...
        try{
            TestData flatTestData = TestDataProvider.getDefaultTestDataProvider().get("/modules/commission/commissionstrategy", "CommissionStrategyGroup5").getTestData("Flat_CommissionRule")
            ...
            createCommissionStrategy(TestDataProvider.getDefaultTestDataProvider().get("/modules/commission/commissionstrategy", "CommissionStrategyGroup5").getTestData("TestDataSTD")
                    .adjust(gbCommissionRuleTab.getMetaKey(), ImmutableList.of(flatTestData)), false);

            reportLog("Add Strategy for Term Life",1);
            createCommissionStrategy(TestDataProvider.getDefaultTestDataProvider().get("/modules/commission/commissionstrategy", "CommissionStrategyGroup5").getTestData("TestDataLife")
                    .adjust(gbCommissionRuleTab.getMetaKey(), ImmutableList.of(flatTestData)), false);

            reportLog("Add Strategy for Dental",1);
            createCommissionStrategy(TestDataProvider.getDefaultTestDataProvider().get("/modules/commission/commissionstrategy", "CommissionStrategyGroup5").getTestData("TestDataDental")
                    .adjust(gbCommissionRuleTab.getMetaKey(), ImmutableList.of(flatTestData)), false);

            reportLog("Agent Name for Group5="+ agencyName, 1);
        }
        catch (Exception e){
