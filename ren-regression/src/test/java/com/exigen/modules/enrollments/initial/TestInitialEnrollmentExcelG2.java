...
import static com.exigen.TestDataKey.DATA_GATHER;
...
public class TestInitialEnrollmentExcelG2 extends CommonBaseTest implements CustomerContext, EfolderContext {
    private static final File FILE = new File(PropertyProvider.getProperty(TestProperties.UPLOAD_FILES_LOCATION), "Initial_Enrollment_Texas.xlsx");
    @Test(priority = 11)
    public void testInitialEnrollmentExcel() throws IOException, InvalidFormatException {
        logger = extent.startTest(groupNo + " TestInitialEnrollmentExcelG2");
        prop.load(fileInput);
        ...
        new Actions(driver).pause(Duration.ofMinutes(20)).perform();
    ...
    public void enrollment(String customerId, File inputFile, int i) throws IOException, InvalidFormatException {
        ...
        CustomerSummaryPage.verifyBamActivitiesContains(772+" records passed validation and 0 records failed", FINISHED);
