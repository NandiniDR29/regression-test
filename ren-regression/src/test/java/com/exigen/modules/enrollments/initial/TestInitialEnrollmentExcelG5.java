...
public class TestInitialEnrollmentExcelG5 extends CommonBaseTest implements CustomerContext, EfolderContext {
    private static final File FILE1 = new File(PropertyProvider.getProperty(TestProperties.UPLOAD_FILES_LOCATION), "Initial Enrollment file75_DE_Life_Den.xlsx");
    private static final File FILE2 = new File(PropertyProvider.getProperty(TestProperties.UPLOAD_FILES_LOCATION), "Initial Enrollment file75_DE_STD.xlsx");
    @Test(priority = 11)
    public void testInitialEnrollmentExcel() throws IOException, InvalidFormatException {
        ...
        String fileName = enrollment(customerId, FILE2,2);
        SaveIEFileNameForNextValidation(fileName,groupNo);
        ...
        for (int i = 0; i< masterPolicies.size(); i++){
            ...
            try {
                RetryService.run(predicate -> PolicySummaryPage.tableEnrollmentProcessingResults.getRow(1).getCell(8).getValue().equals("Completed"),
                        () -> {
                            BrowserController.get().driver().navigate().refresh();
                            PolicySummaryPage.expandEnrollmentProcessingResultsTable();
                            return null;
                        }, StopStrategies.stopAfterAttempt(50), WaitStrategies.fixedWait(60, TimeUnit.SECONDS));
            } catch (RuntimeException e) {
                throw new IstfException(String.format("'Enrollment Status': Expected: 'Completed', but Actual '%s'",
                        PolicySummaryPage.tableEnrollmentProcessingResults.getRow(1).getCell(8).getValue()), e);
            }
            reportLog("Validate EnrollmentProcessingResults", 1);
            try {
    ...
    public String enrollment(String customerId, File inputFile, int i) throws IOException, InvalidFormatException {
