...
public class TestInitialEnrollmentExcelG10 extends CommonBaseTest implements CustomerContext, EfolderContext {
    private static final File FILE = new File(PropertyProvider.getProperty(TestProperties.UPLOAD_FILES_LOCATION), "Initial_Enrollment_IN_Group10.xlsx");
    @Test(priority = 12)
    public void testInitialEnrollmentExcel() throws IOException, InvalidFormatException {
        logger = extent.startTest(groupNo + " TestInitialEnrollmentExcel");
        prop.load(fileInput);
        ...
        searchSpecificParticipant("Jason","5231");
        SaveChildCPNumberForNextValidation(driver.findElements(By.xpath("//td//span[contains(text(),'Group Dental')]/../..//td//a[contains(@id,'act-policies:act-policyList:')]")).get(0).getText());
    }
