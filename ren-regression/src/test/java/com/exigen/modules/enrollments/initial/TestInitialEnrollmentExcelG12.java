...
public class TestInitialEnrollmentExcelG12 extends CommonBaseTest implements CustomerContext, EfolderContext {
    private static final File FILE = new File(PropertyProvider.getProperty(TestProperties.UPLOAD_FILES_LOCATION), "Initial_Enrollment_IN_Group12.xlsx");
    @Test(priority = 11)
        ...
        try {
            RetryService.run(predicate -> generalTab.getAssetList().getAsset(ENROLLMENT_FILE_DIALOG).getAsset(ENROLLMENT_FILE).getAllValues()
                    ...
                    () -> {
                        ...
                        return null;
                    }, StopStrategies.stopAfterAttempt(300), WaitStrategies.fixedWait(300, TimeUnit.SECONDS));
        } catch (RuntimeException e) {
