...
public class Test_3_Downgradeonlay extends ClaimGroupBenefitsDNBaseTest {

    @Test(priority = 101)
    public void Downgradeonlay() throws IOException {


        logger = extent.startTest(groupNo + " Down grade on lay ");
        prop.load(fileInput);
        ...
        search(cpNumber);
        dentalClaim.create(dentalClaim.getDefaultTestData("Filling", "TestData_Claim3"));
        assertThat(ClaimSummaryPage.labelClaimStatus).hasValue(ClaimConstants.ClaimStatus.LOGGED_INTAKE);
        dentalClaim.claimSubmit().perform();
        String claimNumber = ClaimSummaryPage.getClaimNumber();
        reportLog("Claim Submitted successfully for Onlay - porcelain/ceramic - three surfaces " + claimNumber, 1);
        assertThat(ClaimSummaryPage.labelClaimStatus).hasValue(ClaimConstants.ClaimStatus.ADJUDICATED);
        reportLog("Claim Adjudicated for Onlay - porcelain/ceramic - three surfaces ", 1);
        ...
        reportLog("Claim cancelled for Onlay - porcelain/ceramic - three surfaces ", 1);

        dentalClaim.create(dentalClaim.getDefaultTestData("Filling", "TestData_Claim3_1"));
        assertThat(ClaimSummaryPage.labelClaimStatus).hasValue(ClaimConstants.ClaimStatus.LOGGED_INTAKE);
        dentalClaim.claimSubmit().perform();
        String claimNumber1 = ClaimSummaryPage.getClaimNumber();
        reportLog("Claim Submitted successfully for Inlay - metallic - one surface [Downgrade onlay] " + claimNumber1, 1);
        reportLog("-----------------------------------------", 1);
    }
}
