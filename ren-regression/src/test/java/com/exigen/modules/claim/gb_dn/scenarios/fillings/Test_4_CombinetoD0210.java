...
public class Test_4_CombinetoD0210 extends ClaimGroupBenefitsDNBaseTest {

    @Test(priority = 102)
    public void CombinetoD0210() throws IOException {


        logger = extent.startTest(groupNo + " Combine to D0210");
        prop.load(fileInput);
        ...
        search(cpNumber);
        dentalClaim.create(dentalClaim.getDefaultTestData("Filling", "TestData_Claim4"));
        assertThat(ClaimSummaryPage.labelClaimStatus).hasValue(ClaimConstants.ClaimStatus.LOGGED_INTAKE);
        dentalClaim.claimSubmit().perform();
        String claimNumber = ClaimSummaryPage.getClaimNumber();
        reportLog("Claim Submitted successfully for Intraoral - periapical first radiographic image " + claimNumber, 1);
        ...
        reportLog("Claim cancelled for Intraoral - periapical first radiographic image ", 1);

        dentalClaim.create(dentalClaim.getDefaultTestData("Filling", "TestData_Claim4_1"));
        assertThat(ClaimSummaryPage.labelClaimStatus).hasValue(ClaimConstants.ClaimStatus.LOGGED_INTAKE);
        dentalClaim.claimSubmit().perform();
