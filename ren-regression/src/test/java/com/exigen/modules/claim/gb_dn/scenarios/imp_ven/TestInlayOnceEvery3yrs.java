...
public class TestInlayOnceEvery3yrs extends ClaimGroupBenefitsDNBaseTest {

    @Test(priority = 115)
    public void testInlayOnceEvery3yrs() throws IOException {


        logger = extent.startTest(groupNo + " testInlayOnceEvery3yrs");
        prop.load(fileInput);
        ...
        dentalClaim.create(dentalClaim.getDefaultTestData("ImpVen", "TestData_Inlayonceevery3yrs"));
        assertThat(ClaimSummaryPage.labelClaimStatus).hasValue(ClaimConstants.ClaimStatus.LOGGED_INTAKE);
        dentalClaim.claimSubmit().perform();
        String claimNumber = ClaimSummaryPage.getClaimNumber();
        reportLog("Claim Submitted successfully for Inlay Once Every 3 yrs " + claimNumber, 1);
        reportLog("Claim should pay and downgrade to composite", 1);

        dentalClaim.create(dentalClaim.getDefaultTestData("ImpVen", "TestData_Inlayonceevery3yrsDeny"));
        assertThat(ClaimSummaryPage.labelClaimStatus).hasValue(ClaimConstants.ClaimStatus.LOGGED_INTAKE);
        dentalClaim.claimSubmit().perform();
        String claimNumber1 = ClaimSummaryPage.getClaimNumber();
        reportLog("Claim Submitted successfully for Inlay Once Every 3 yrs deny scenario " + claimNumber1, 1);
