...
public class TestCorpCustomerCreationNJ extends CustomerBaseTest {
    @Test(priority = 1)
    public void testCorpNJCustomerCreation() throws IOException {
        logger = extent.startTest(groupNo+" Corp NJ Customer creation");
        prop.load(fileInput);
        ...
        AutomationContext.getAction(AddCustomerAction.class).getWorkspace().fillUpTo(tdCustomerNonIndividual.getTestData("CorpDataGather", "TestData_NJ").adjust(TestData.makeKeyPath(GeneralTab.class.getSimpleName(), EIN.getLabel()), createEIN())
