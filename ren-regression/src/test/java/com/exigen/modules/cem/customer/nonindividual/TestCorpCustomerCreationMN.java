...
public class TestCorpCustomerCreationMN extends CustomerBaseTest {
    @Test(priority = 1)
    public void testCorpMNCustomerCreation() throws IOException {
        logger = extent.startTest(groupNo+" Corp MN Customer creation");
        prop.load(fileInput);
        ...
        AutomationContext.getAction(AddCustomerAction.class).getWorkspace().fillUpTo(tdCustomerNonIndividual.getTestData("CorpDataGather", "TestData_MN").adjust(TestData.makeKeyPath(GeneralTab.class.getSimpleName(), EIN.getLabel()), createEIN())
