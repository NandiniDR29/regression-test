...
import static com.exigen.helpers.CommonGenericMethods.saveCustomerName;
...
public class TestCorpCustomerCreationDE extends CustomerBaseTest {
    @Test(priority = 1)
    public void testCorpDECustomerCreation() throws IOException {
        logger = extent.startTest(groupNo+" Corp DE Customer creation");
        prop.load(fileInput);
        ...
        AutomationContext.getAction(AddCustomerAction.class).getWorkspace().fillUpTo(tdCustomerNonIndividual.getTestData("CorpDataGather", "TestData_DE").adjust(TestData.makeKeyPath(GeneralTab.class.getSimpleName(), EIN.getLabel()), createEIN())
        ...
        saveCustomerName(CustomerSummaryPage.labelCustomerName.getValue());
        reportLog("CustomerName: "+CustomerSummaryPage.labelCustomerName.getValue(),1);
        reportLog("==================================", 1);
    }
}
