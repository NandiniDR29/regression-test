...
public class TestCorpCustomerCreationFL extends CustomerBaseTest {
    @Test(priority = 1)
    public void testCorpFLCustomerCreation() throws IOException {
        logger = extent.startTest(groupNo+" Corp FL Customer creation");
        prop.load(fileInput);
        ...
        AutomationContext.getAction(AddCustomerAction.class).getWorkspace().fillUpTo(tdCustomerNonIndividual.getTestData("CorpDataGather", "TestData_FL").adjust(TestData.makeKeyPath(GeneralTab.class.getSimpleName(), EIN.getLabel()), createEIN())
