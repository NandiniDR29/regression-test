...
public class TestCorpCustomerCreationTX extends CustomerBaseTest {
    @Test(priority = 1)
    public void testCorpTXCustomerCreation() throws IOException {
        logger = extent.startTest(groupNo+" Corp TX Customer creation");
        prop.load(fileInput);
        ...
        AutomationContext.getAction(AddCustomerAction.class).getWorkspace().fillUpTo(tdCustomerNonIndividual.getTestData("CorpDataGather", "TestData_TX").adjust(TestData.makeKeyPath(GeneralTab.class.getSimpleName(), EIN.getLabel()), createEIN())
