...
public class CoveragesTab extends DefaultTab {
                ...
                for (TestData coverage : td.getTestDataList(name)) {
                    if (coverage.containsKey("Action")) {
                        if (coverage.getValue("Action").equals("Add Coverage")) {
                            ...
                            tableCoveragesList.getRow(TableConstants.CertificateCoverages.COVERAGE_NAME.getName(), coverage.getValue(CoveragesTabMetaData.COVERAGE_NAME.getLabel())).getCell(tableCoveragesList.getColumnsCount()).controls.links.get(ActionConstants.CHANGE).click();
                        } else if (coverage.getValue("Action").equals("Remove")) {
                            tableCoveragesList.getRow(TableConstants.CertificateCoverages.COVERAGE_NAME.getName(), coverage.getValue(CoveragesTabMetaData.COVERAGE_NAME.getLabel())).getCell(tableCoveragesList.getColumnsCount()).controls.links.get(ActionConstants.REMOVE).click();
                        }
    ...
    public static void openAddedPlan(String planName) {
        ...
        tableCoveragesList.getRowContains(TableConstants.CertificateCoverages.COVERAGE_NAME.getName(), planName).getCell(5).controls.links.get(ActionConstants.CHANGE).click();
    }

    public static void removeExistingPlan(String planName) {
        LOGGER.info(String.format("Remove Plan: '%s'", planName));
        tableCoveragesList.getRowContains(TableConstants.CertificateCoverages.COVERAGE_NAME.getName(), planName).getCell(5).controls.links.get(ActionConstants.REMOVE).click();
    ...
    public static void openExistingBeneficiary(String cellValue) {
        ...
        tableBeneficiariesList.getRowContains(TableConstants.CertificateBeneficiaries.ROLE_PERCENT.getName(), cellValue).getCell(6).controls.links.get(ActionConstants.CHANGE).click();
    }
    ...
    public static void openExistingBeneficiaryByRole(String role) {
        ...
        tableBeneficiariesList.getRowContains(TableConstants.CertificateBeneficiaries.ROLE_NAME.getName(), role).getCell(6).controls.links.get(ActionConstants.CHANGE).click();
    }
    ...
    public static void openExistingParticipant(String cellValue) {
        ...
        tableParticipantsList.getRowContains(RELATIONSHIP_TO_INSURED.getName(), cellValue).getCell(6).controls.links.get(ActionConstants.CHANGE).click();
    }
